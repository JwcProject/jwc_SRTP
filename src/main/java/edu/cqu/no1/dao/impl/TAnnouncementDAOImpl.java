...
public class TAnnouncementDAOImpl extends BaseDaoImpl<TAnnouncement> implements edu.cqu.no1.dao.TAnnouncementDAO {
    ...
    public TAnnouncementModel getAnnounById(String announId) {
        ...
        try {
            // TODO: Check whether if the following hql omit `ta.isdeleted = 'N'` in `where` clause.
    ...
    public List findUnitAnnoun(String teaCode, String announName,
        ...
        try {
            ...
            hql +=  " and ta.publisherCode = tu.userId and ta.publisherCode in (select" +
                    " teaCode from TTeacher where unitId = (select unitId from TTeacher  where teaCode = :code))" +
                    " or ta.publisherCode in (select studentNumber from TStudent  where unitId = (select" +
                    " unitId from TTeacher where teaCode = :code))";
            Query query = getSessionFactory().getCurrentSession().createQuery(hql);
    ...
    public List findSchoolAnnoun(String announName, String checkState,
        ...
        try {
            ...
            if (null != typeName && !typeName.trim().equals("00")) {
                hql += " and ta.announTypeId = (select announTypeId from TAnnounType where announTypeName like :tName)";
            }
    ...
    public int findSchoolAnnounCount(String announName, String checkState,
        ...
        try {
            ...
            if (null != checkState && !checkState.trim().equals("") && !checkState.trim().equals("ALL")) {
                hql += " and checkState = :aState";
            }
            if (null != checkState && checkState.equals("ALL")) {
                hql += " and checkState is not null";
            }
            if (null != announDate && !announDate.toString().trim().equals("")) {
                hql += " and publishTime between :aDate and :aDate + 1";
            }
            if (null != publisherName && !publisherName.trim().equals("")) {
                hql += " and publisherCode in (select userId from TUser where userName like :uName)";
            }
            if (null != typeName && !typeName.trim().equals("00")) {
                hql += " and announTypeId = (select announTypeId from TAnnounType  where announTypeName like :tName)";
            }
            Query query = getSessionFactory().getCurrentSession().createQuery(hql);
            if (null != announName && !announName.trim().equals("")) {
                query.setString("aName", "%" + announName + "%");
    ...
    public List getUnitAnnounctment(String unitTeaCode, PageBean pageBean) {
        ...
        try {
            String hql = "select new TAnnouncementModel(ta, tu.userName) from TAnnouncement ta, TUser tu where" +
                    " ta.isdeleted = 'N' and ta.publisherCode = tu.userId and ta.publisherCode in (select" +
                    " teaCode from TTeacher where unitId = (select unitId from TTeacher where teaCode = :code))" +
                    " or ta.publisherCode in (select studentNumber from TStudent where unitId = (select" +
                    " unitId from TTeacher where teaCode = :code))";
            Query query = getSessionFactory().getCurrentSession().createQuery(hql);
    ...
    public int getUnitAnnounctmentCount(String unitTeaCode) {
        ...
        try {
            String hql = "select count(*) from TAnnouncement where isdeleted = 'N' and publisherCode in (select" +
                    " teaCode from TTeacher where unitId = (select unitId from TTeacher where teaCode = :code))" +
                    " or publisherCode in (select studentNumber from TStudent where unitId = (select unitId" +
                    " from TTeacher where teaCode = :code))";
            Query query = getSessionFactory().getCurrentSession().createQuery(hql);
            query.setString("code", unitTeaCode);
            List tmpList = query.list();
            int count = 0;
    ...
    public List getSchoolAnnounctment(PageBean pageBean) {
        ...
        try {
            String hql = "select new TAnnouncementModel(t, tu.userName) from TAnnouncement  t, TUser tu" +
                    " where t.isdeleted = 'N' and t.publisherCode = tu.userId and t.checkState is not null" +
    ...
    public int getSchoolAnnounctmentCount() {
        ...
        try {
            String hql = "select count(*) from TAnnouncement where isdeleted ='N' and checkState is not null";
            Query query = getSessionFactory().getCurrentSession().createQuery(hql);
            List tmpList = query.list();
            int count = 0;
            if (tmpList.size() > 0) {
    ...
    public List findIndexSchoolAnnoument(String announTitle, Date publishTime, String typeName, PageBean pageBean) {
        log.debug("find TAnnouncement by type");
        try {
            String hql = "select new TAnnouncementModel(t, tu.userName) from TAnnouncement t, TAnnounType ta, TUser tu where" +
                    " t.isdeleted = 'N' and t.publishState = 'Y' and t.publisherCode = tu.userId" +
                    " and t.announTypeId = ta.announTypeId and ta.announTypeName like :tName";

            if (null != announTitle && !announTitle.trim().equals("")) {
                hql += " and t.announTitle like :announTitle";
            }
            if (null != publishTime && !publishTime.toString().trim().equals("")) {
                hql += " and t.publishTime between :publishTime and :publishTime + 1";
            }
    ...
    public int findIndexSchoolAnnoumentCount(String announTitle, Date publishTime, String typeName) {
        log.debug("find TAnnouncement count by type");
        try {
            String hql = "select count(*) from TAnnouncement t, TAchievemenType ta where" +
                    " t.isdeleted = 'N' and t.publishState = 'Y' and t.announTypeId = ta.achievementypeId" +
                    " and ta.achievementypeName like :tName";
