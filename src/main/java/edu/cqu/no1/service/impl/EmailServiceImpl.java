...
public class EmailServiceImpl implements EmailService {
    ...
    public boolean saveEmail(MailInfo mailInfo, boolean sendState) {
        ...
        List userRoleList = tUserRoleDAO.findByProperty("TUser", user);
        if (userRoleList.size() == 0) {
            return false;
        }
        TUserRole userRole = (TUserRole) userRoleList.get(0);

        TRole role = tRoleDAO.findById(userRole.getTRole().getRoleId());
        ...
        receiver.setReceiverRole(roleName);
